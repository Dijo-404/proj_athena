...
async function loadLocale(languageOverride) {
  ...
  try {
    ...
    if (version !== loadLocaleVersion) {
      return;
    }
    applyLocale(strings);
  } catch (error) {
    console.warn(`Locale ${language} failed to load`, error);
  }
}
...
When filling forms, proceed step by step. The user must approve EACH fill_field action via a UI prompt, so:
...
Think step by step before making eligibility decisions.`;
}
...
function requestUserApproval(args) {
  return new Promise((resolve) => {
    ...
    const isSubmit =
      args?.action === "click" &&
      /submit|apply|send|saméry|பதிவு|சமர்ப்பி/i.test(
        String(args?.field_label || ""),
      );
    if (isSubmit) {
      card.dataset.kind = "submit";
    }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  requestUserApproval(message.payload || {}).then((result) => {
    sendResponse(result);
  });
  return true;
});
